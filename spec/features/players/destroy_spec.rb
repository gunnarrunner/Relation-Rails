...
RSpec.describe 'Can destroy the player and delete them from the table' do
  before :each do
    ...
    visit "/players/#{@player17.id}"
  end

  it 'can delete an artist from the show page' do
    click_on("Delete #{@player17.name}")

    expect(current_path).to eq("/players")
    expect(page).to_not have_content("#{@player17.name}")
  end
end